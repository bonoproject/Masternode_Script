#!/bin/bash
# BonorumCoin Masternode Setup Script V1 for Ubuntu 18.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'bonorumcoind' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop bonorumcoind${NC}"
        bonorumcoin-cli stop
        sleep 30
        if pgrep -x 'bonorumcoind' > /dev/null; then
            echo -e "${RED}bonorumcoind daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 bonorumcoind
            sleep 30
            if pgrep -x 'bonorumcoind' > /dev/null; then
                echo -e "${RED}Can't stop bonorumcoind! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${GREEN} ------- BonorumCoin MASTERNODE INSTALLER v1.0.0--------+
 ...
 |        the masternode under a user bonorumcoin.         |::
 |                                                  |::
...
wget https://github.com/bonoproject/Bonorum/releases/download/v1.0/Bonorumcoin-daemon-ubuntu_18.04.tar.gz
tar -xzvf Bonorumcoin-daemon-ubuntu_18.04.tar.gz
...
======================================================================== \a"
echo -e "${GREEN}bonorumcoin_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}bonorumcoin_mn1${NC} - with your desired masternode name (alias)
...
echo -e "${NC}-------------------------------------------------
NOTE: To edit bonorumcoin.conf, first stop the bonorumcoind daemon,
then edit the bonorumcoin.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the bonorumcoind daemon back up:
